...
> MuonTrap can certainly run an LLM, but sandboxing it to limit network
> access, for example, is not supported. Look at tools like
...
`MuonTrap.Daemon.statistics/1` returns the daemon's output counters plus a
snapshot of every readable cgroup stat file under a `:cgroup` key. The
cgroup map is keyed by the v2 interface file name (memory usage and peak,
CPU and memory PSI, OOM-kill counts, `pids.current`, etc.):
