...
defmodule DaemonTest do
  ...
  test "can start daemon with cgroups" do
    ...
    assert is_integer(stats.cgroup["memory.current"]) and stats.cgroup["memory.current"] >= 0
    assert is_integer(stats.cgroup["memory.events"]["oom_kill"])
    assert is_integer(stats.cgroup["cpu.stat"]["usage_usec"])
